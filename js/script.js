...
function fetch_tweets(elem){
	...
	var url = "http://search.twitter.com/search.json?q=%23drinkup&rpp=20&callback=?"
	$.getJSON(url,function(json){
		...
		$(json.results).each(function(){
			...
			var tweet='<div class="tweet"><div class="tweet-left"><a target="_blank" href="http://twitter.com/'+this.from_user+'"><img width="48" height="48" alt="'+this.from_user+' on Twitter" src="'+this.profile_image_url+'" /></a></div><div class="tweet-right"><a class="tweet-user" target="_blank" href="http://twitter.com/'+this.from_user+'">'+this.from_user+': </a><span class="text">'+this.text.linkify().linkuser().linktag().replace(/<a/g,'<a target="_blank"')+'</span><br/><span class="tweet-time">'+since+'</span><br style="clear: both;" /></div></div>';
...
function update_tweets(elem){
	...
	// re-enable for generic
	// var url="http://search.twitter.com/search.json?q="+keyword+"&rpp="+num+"&callback=?";
	var url = "http://search.twitter.com/search.json?q=%23drinkup&rpp=20&since_id="+max_id+"&callback=?"
	$.getJSON(url,function(json){
		max_id = json.max_id_str
		$(json.results).each(function(){
			var tTime=new Date(Date.parse(this.created_at));
